import 'entry.dart';

class ListDB {
  static List<Entry> listDb = [
    Entry(
        Id: '2313',
        Subject: 'English',
        Grade: 'rand',
        Amount: '9/10',
        Room: () {}),
    Entry(Id: '2314', Subject: 'Math', Grade: '9', Amount: '8/10', Room: () {}),
    Entry(
        Id: '2315',
        Subject: 'Geography',
        Grade: '9',
        Amount: '10/10',
        Room: () {}),
    Entry(
        Id: '2316',
        Subject: 'Math',
        Grade: 'rand',
        Amount: '8/10',
        Room: () {}),
    Entry(Id: '2317', Subject: 'Math', Grade: '9', Amount: '6/6', Room: () {}),
    Entry(
        Id: '2318',
        Subject: 'Physics',
        Grade: '9',
        Amount: '6/10',
        Room: () {}),
    Entry(Id: '2319', Subject: 'Math', Grade: '9', Amount: '2/2', Room: () {}),
  ];
}

class ListDbPlayLevels {
  static List<PlayLevels> listPlayLevels = [
    PlayLevels(Level: "1", Topic: "World Cup 2022"),
    PlayLevels(Level: "2", Topic: "Chủ đề 2"),
    PlayLevels(Level: "3", Topic: "Chủ đề 3"),
    PlayLevels(Level: "4", Topic: "Chủ đề 4"),
    PlayLevels(Level: "5", Topic: "Chủ đề 5"),
    PlayLevels(Level: "6", Topic: "Chủ đề 6"),
    PlayLevels(Level: "7", Topic: "Chủ đề 7"),
    PlayLevels(Level: "8", Topic: "Chủ đề 8"),
    PlayLevels(Level: "9", Topic: "Chủ đề 9"),
    PlayLevels(Level: "10", Topic: "Chủ đề 10")
  ];
}
